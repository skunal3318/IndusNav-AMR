...
class BatterySimulator(Node):
    def __init__(self):
        ...
        self.declare_parameter('robot_voltage', 24.0)
        self.declare_parameter('capacity', 5.0)
        ...
        self._voltage = self.get_parameter('robot_voltage').value
        self._capacity = self.get_parameter('capacity').value
        ...
        def _charging_cb(self, msg: Bool):
            if msg.data != self._is_charging:
                ...
                self.get_logger().info(
                    'Charging' + ('Started' if self.self._is_charging else 'Stopped')
                )
